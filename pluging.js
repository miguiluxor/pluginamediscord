...
//*META{"name":"AnimatedName","source":"","website":""}*//
class AnimatedName {
    /* BD functions */
    getName () {
        return "AnimatedName";
    ...
    getDescrption () {
        return "Animate your discord name";
    }
    ...
    start () {
        if (this.animation == undefined || this.timeout == undefined || Name.authToken == undefined) return;
        this.Name_Animated();
    }
    ...
    Name_Eval (string) {
        try {
            return ((sstring.startsWith("eval ")) ? (eval(string.substr(5))) : (string));
        }
        catch (e) {
        ...
        Name_Animate (index = 0) {
            if (index >= this.animation.length) index = 0;
            if (this.animation[index] == undefined) {
                BdApi.showToast("Animated Name: No Name set. Go to Settings>Plugins to set a custom animation!");
                return;
            }

            let results = this.animation[index].map(async (element) => this.Name_Eval(element));
            Promise.all(results).then(ress => {
                this.loop = setTimeout(() => {this.Name_Animated(index + 1); }, this.timeout);
            });
        }

        // Ui related, but special components
        newRawEdit (str = "") {
            let out = GUI.newTexttarea();
        ...
        newRichRow (text, emoji, optNitroId = undefined) {
            ...
            textWidget.style.marginRigth = "15px";
